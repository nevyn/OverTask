...
struct {
	...
	{kVK_ANSI_F, ^(id task){ [task focusSelected:nil]; } },
	{kVK_ANSI_D, ^(id task){ [task unfocus:nil]; } },
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification;
{
	...
	for(int i = 0; OTKeys[i].last != YES; i++) {
		s = RegisterEventHotKey(OTKeys[i].key, cmdKey|optionKey|controlKey, nul, t, 0, &OTKeys[i].keyref);
	...
	NSEvent*(^flagsHandler)(NSEvent*) = ^(NSEvent *evt) {
		...
		if(newFlags == (NSCommandKeyMask|NSAlternateKeyMask|NSControlKeyMask)) {
